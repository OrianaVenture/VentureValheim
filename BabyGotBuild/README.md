# Baby Got Build (Favorites)

Created by [OrianaVentureMod@gmail.com](https://github.com/OrianaVenture/VentureValheim).

## Introduction

Saves all your build favorites to a new file so it can synchronize and share across all your different characters. You can also send your favorites to a friend!
...
1. Load the character with your current build favorites into a world.
2. Log out. A new file will save to your character folder called ``BabyGotBuildData.extras``.
3. All future log ins will now load from and save to this same favorites list.

On save one copy of the old data is kept if you ever need to restore a backup. Just delete the ``.extras`` file and remove the ``.old`` from the previous version.

This favorites file can be shared and loaded by anyone with the mod installed!

## Installation

This mod is client side only and has no configurations. Removing the mod will prevent loading any saved data, but will not break anything. To remove the files generated by this mod they must be manually deleted.
